# MiniMax 持续采集任务：minimax-all-18-scholarships

先完整读取仓库根目录的 `MINIMAX_DATA_COLLECTION_PROMPT.md`，再执行本任务。

## 锁定参数

- `TASK_ID`: `minimax-all-18-scholarships`
...
- `quality/minimax-harvest/inbox/minimax-all-18-scholarships.json`
- `quality/minimax-harvest/inbox/minimax-all-18-scholarships.md`

JSON 的 `batchId` 必须等于 `minimax-all-18-scholarships`，`scope.schoolIds` 必须按顺序包含：
...
```powershell
npx tsx scripts/ingestion/validate-minimax-harvest.ts --task minimax-all-18-scholarships
```
