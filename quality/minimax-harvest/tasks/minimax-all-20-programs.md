# MiniMax 持续采集任务：minimax-all-20-programs

先完整读取仓库根目录的 `MINIMAX_DATA_COLLECTION_PROMPT.md`，再执行本任务。

## 锁定参数

- `TASK_ID`: `minimax-all-20-programs`
- `TASK_KIND`: `programs`
- `CHECKED_AT`: 使用实际运行日期
- `SCHOOL_LIMIT`: `8`
...
- `quality/minimax-harvest/inbox/minimax-all-20-programs.json`
- `quality/minimax-harvest/inbox/minimax-all-20-programs.md`

JSON 的 `batchId` 必须等于 `minimax-all-20-programs`，`scope.schoolIds` 必须按顺序包含：
...
```powershell
npx tsx scripts/ingestion/validate-minimax-harvest.ts --task minimax-all-20-programs
```
