...
registry and the current catalog JSON into a review queue for
`SourceManifestV2`. It does not discover new URLs, fetch websites, or publish
manifests.

The separation is intentional:

`official target registry + current catalog relationships -> candidate artifact -> evidence review -> formal manifest`
...
- `artifact-manifest.v1.json`: exact SHA-256 and byte length for all six
  locked inputs and every generated JSON file;
...
`runner.temp`, validates the official target registry, and validates the
relevant current-catalog relationships while building each disabled candidate.
