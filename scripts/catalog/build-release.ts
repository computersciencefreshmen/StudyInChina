...
export type ReleaseArtifacts = {
  ...
  compatibilityByteLength: number
  r2Key: string
  sql: string
  envelope: string
}
...
function localizedValue(value: LocalizedText, locale: string): string | null {
  return value[locale as keyof LocalizedText]
    ?? value.en
...
function addSearch(
  ...
  for (const locale of new Set([...Object.keys(title), ...Object.keys(body ?? {})])) {
    ...
    if (!localizedTitle) throw new Error(`${recordId} has no searchable title`)
    statements.push(insert('search_documents', {
      ...
      body: body ? localizedValue(body, locale) ?? '' : '',
...
export function buildLegacyRelease(bundleInput: DataBundle): ReleaseArtifacts {
  ...
  const compatibilityByteLength = new TextEncoder().encode(envelope).byteLength
  const r2Key = `releases/${releaseId}/compat-envelope.json`
  const statements: string[] = [
    ...
    insert('release_compatibility_artifacts', {
      release_id: releaseId,
      artifact_format: 'studyinchina.frontend.bundle.v1',
      artifact_key: r2Key,
      content_sha256: contentSha256,
      byte_length: compatibilityByteLength,
      created_at: generatedAt,
    }),
  ]
...
function main() {
  ...
  writeFileSync(manifestPath, JSON.stringify({
    ...
    compatibilityByteLength: artifacts.compatibilityByteLength,
