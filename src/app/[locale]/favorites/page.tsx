...
export default async function FavoritesPage({ params }: { params: Promise<{ locale: string }> }) { const locale = requireLocale((await params).locale); if (!locale) notFound(); const messages = getMessages(locale); const data = getData(); return <><PageHero variant="compact" eyebrow={messages.common.compare} title={messages.favorites.title} description={messages.favorites.intro} /><section className="atlas-container atlas-section"><FavoritesView programs={data.programs} universities={data.universities} cycles={data.admissionCycles} locale={locale} messages={messages} /></section></> }
