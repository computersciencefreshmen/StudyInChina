...
export type CoverageInstitution = {
  ...
  status: 'source_manifest_complete' | 'collecting'
          ...
          <tbody>
            {filtered.map((institution) => {
              const sources = new Map(
                institution.sources.map((source) => [source.category, source]),
              )
              ...
              return (
                <tr key={institution.targetId}>
                  ...
                  <td>
                    <span className={[
                      styles.status,
                      complete ? styles.statusComplete : styles.statusCollecting,
                    ].join(' ')}>
                      {complete ? `3/3 · ${copy.complete}` : copy.collecting}
                    </span>
                  </td>
                        ...
                        {sourceOrder.map((category) => {
                          ...
                          return source ? (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              title={source.verificationMethod}
                            >
