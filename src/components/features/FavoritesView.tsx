...
export function FavoritesView({ programs, universities, cycles, locale, messages, today }: { programs: Program[]; universities: University[]; cycles: AdmissionCycle[]; locale: LaunchLocale; messages: Messages; today: string }) {
  ...
  return <div className="atlas-stack" style={{ '--atlas-stack-gap': '3rem' } as React.CSSProperties}>
    ...
    {compared.length ? <section><div className="record-card__top"><h2>{copy.comparison}</h2><Button className="no-print" variant="ghost" onClick={() => window.print()}>{messages.common.print}</Button></div><div className="compare-grid">{compared.map((program) => { const university = universities.find((item) => item.id === program.universityId); const cycle = selectAdmissionCycle(cycles, program.id, today); return <Card key={program.id} accent="jade"><h3 className="atlas-card__title">{localize(program.name, locale)}</h3><dl className="compare-facts"><div><dt>{copy.university}</dt><dd>{university ? localize(university.name, locale) : '—'}</dd></div><div><dt>{messages.programs.degree}</dt><dd>{degreeLabels(locale)[program.degreeLevel]}</dd></div><div><dt>{messages.common.language}</dt><dd>{program.teachingLanguages.length ? program.teachingLanguages.map((item) => languageLabel(item, locale)).join(', ') : messages.common.unknown}</dd></div><div><dt>{messages.common.duration}</dt><dd>{durationLabel(program, messages)}</dd></div><div><dt>{messages.common.tuition}</dt><dd>{tuitionLabel(cycle, locale, messages)}</dd></div><div><dt>{messages.common.deadline}</dt><dd>{formatDate(cycle?.closesOn ?? null, locale, messages.common.unknown)}</dd></div></dl></Card> })}</div></section> : null}
  </div>
}
