...
以下指标来自本地真实执行的 `npm run quality:platform-scorecard`，评估日期为 **2026-08-17**。它描述的是当前公开发布视图，不等同于“所有字段都完整”或“所有项目都正在开放申请”。

<div align="center">

| **266** 所高校 | **1,233** 个项目 | **350** 项奖学金 | **62** 座城市 |
| :------------: | :--------------: | :--------------: | :-----------: |

</div>

当前公开视图包含 **52** 条招生周期或学费参考记录。目录背后登记了官方来源与证据管线；但只有完成发布门禁的字段才会显示为具体值。项目身份、当前招生动态、费用、要求和奖学金资格是不同层次的数据，不能用一条总数混为一谈。

### 数据质量口径：先诚实，再扩张

行数不是数据深度。当前分数卡将“项目是否存在”与“该项目的本期事实是否已经核验”分开统计：

| 指标                               |                   当前基线 |                        近期目标 |
| ---------------------------------- | -------------------------: | ------------------------------: |
| 少于 3 个公开项目的高校            |                   **8** 所 | 0 所，或完成 `limited` 目录对账 |
| 已核验的国际生项目身份             |   1,233 / 1,233 · **100%** |                       维持 100% |
...
| 关联至少一项奖学金的高校           |     204 / 266 · **76.69%** |                        ≥ 230 所 |
...
补充说明：当前有 **42 / 350（12%）** 奖学金记录带明确截止日；有 **1** 条已核验事实进入复核期后尚未完成新的处置。它不会被包装成“高完整度”，而是会进入维护优先级。`published` 且完全没有日期的招生周期记录为 **0**。
