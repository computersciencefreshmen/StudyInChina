...
公开 API 的身份目录与本地真实执行的 `npm run quality:platform-scorecard` 使用不同且互补的口径，评估日期均为 **2026-08-25**：前者保留可发现的已核验或 `stale` 身份，后者只统计当前新鲜证据。两者都不等同于“所有字段都完整”或“所有项目都正在开放申请”。

<div align="center">

| **266** 所高校 | **1,245** 个项目身份 | **364** 项奖学金身份 | **62** 座城市 |
| :------------: | :------------------: | :------------------: | :-----------: |

</div>

当前公开身份目录包含 **37** 条当前已发布招生周期记录，并由 **2,078** 条已登记官方来源记录支撑；其中只有 **6** 条在评估日处于开放、即将开放或滚动招生状态。“已发布”不等于“正在开放申请”。目录保留可发现的已核验或 `stale` 身份；下方质量分数卡则只统计当前新鲜证据。项目身份、当前招生动态、费用、要求和奖学金资格是不同层次的数据，不能用一条总数混为一谈。
...
| 指标                               |                   当前基线 |                        近期目标 |
| ---------------------------------- | -------------------------: | ------------------------------: |
| 少于 3 个公开项目的高校            | **3** 所 · 1 所已记录 `limited` |                    0 所未处置 |
| 当前新鲜核验的国际生项目身份       | 1,239 / 1,245 · **99.52%** |                     恢复至 100% |
| 30 天内有新鲜处置结论的项目        |     37 / 1,245 · **2.97%** |                           ≥ 70% |
| 有明确日期或滚动招生语义的项目     |     37 / 1,245 · **2.97%** |          如实报告，不用猜测补齐 |
| 评估日处于开放或即将开放状态的项目 |      6 / 1,245 · **0.48%** |        如实报告，不用旧日期抬高 |
| 已知学制的项目                     |   765 / 1,245 · **61.45%** |                           ≥ 90% |
| 有官方申请入口的项目               |   630 / 1,245 · **50.60%** |                           ≥ 80% |
| 已知授课语言的项目                 | 1,055 / 1,245 · **84.74%** |                           ≥ 95% |
| 有资格或语言要求证据的项目         |     76 / 1,245 · **6.10%** |                           ≥ 50% |
...
原始兼容数据集包含 272 所高校、1,266 个项目和 388 项奖学金。草稿、归档记录、身份冲突记录或尚不满足公开条件的候选，会被有意排除在上述公开数字之外。数据少于预期时，优先意味着系统在拒绝猜测，而不是把空白伪装成事实。

公开 Release API 返回的是更宽的 **1,245** 个项目身份和 **364** 项奖学金身份；分数卡则有意返回较小的当前新鲜核验子集。两种指标都必须保留，不能互相替换。
...
- 解决湖南工商大学和浙江大学两所剩余扩充目标；西藏大学保留带证据的 `limited` 官方目录状态，不用缺乏官方依据的身份填数。
- 将新鲜处置覆盖率从 2.97% 提升到至少 70%，不把无日期学费参考计为招生周期。
