...
> [!NOTE]
> **可复现目录基线 · 2026-08-26：**266 所公开高校、1,263 个项目身份、367 项奖学金身份。运行 `npm run quality:platform-scorecard` 可复现质量指标；[Release API](https://studyinchina.vercel.app/api/v1/releases/current) 提供线上发布元数据。本文件与 [English README](./README.md) 同步维护；若文案与可执行校验不一致，以数据校验和官方证据为准。
...
| **266** 所高校 | **1,263** 个项目身份 | **367** 项奖学金身份 | **62** 座城市 |
| :------------: | :------------------: | :------------------: | :-----------: |

</div>

截至 **2026-08-26**，当前公开身份目录包含 **43** 条公开招生周期记录，并由 **2,130** 条已登记官方来源记录支撑；其中只有 **8** 个项目身份处于开放、即将开放或滚动招生状态。“公开周期”不等于“正在开放申请”。目录保留可发现的已核验或 `stale` 身份；下方质量分数卡则单独衡量当前新鲜证据。项目身份、招生动态、费用、要求和奖学金资格属于不同数据层，不能用一条总数混为一谈。
...
| 指标                               |                   当前基线 |                        近期目标 |
| ---------------------------------- | -------------------------: | ------------------------------: |
| 少于 3 个公开项目的高校            | **2** 所 · 1 所已记录 `limited` |                    0 所未处置 |
| 通过身份发布门禁的国际生项目       | 1,263 / 1,263 · **100%** |                     保持 100% |
| 30 天内有新鲜处置结论的项目        |     42 / 1,263 · **3.33%** |                           ≥ 70% |
| 有明确日期或滚动招生语义的项目     |     42 / 1,263 · **3.33%** |          如实报告，不用猜测补齐 |
| 评估日开放、即将开放或滚动招生的项目 |      8 / 1,263 · **0.63%** |        如实报告，不用旧日期抬高 |
| 有当前已确认学费的项目             |      7 / 1,263 · **0.55%** |                如实报告当前事实 |
| 有任一年度官方学费证据的项目       |    538 / 1,263 · **42.60%** |          仅作证据，不冒充当前值 |
| 已知学制的项目                     |   801 / 1,263 · **63.42%** |                           ≥ 90% |
| 有官方申请入口的项目               |   588 / 1,263 · **46.56%** |                           ≥ 80% |
| 已知授课语言的项目                 |   989 / 1,263 · **78.31%** |                           ≥ 95% |
| 有资格或语言要求证据的项目         |   142 / 1,263 · **11.24%** |                           ≥ 50% |
| 关联至少一项新鲜核验奖学金的高校   |     205 / 266 · **77.07%** |                        ≥ 230 所 |
| 有明确截止日的新鲜奖学金           |      38 / 348 · **10.92%** |                持续补齐当前周期 |
...
原始兼容数据集包含 272 所高校、1,280 个项目和 394 项奖学金。草稿、归档记录、身份冲突记录或尚不满足公开条件的候选，会被有意排除在上述公开数字之外。数据少于预期时，优先意味着系统在拒绝猜测，而不是把空白伪装成事实。

公开 Release API 与分数卡的身份口径一致，均为 **1,263** 个项目身份和 **367** 项奖学金身份；分数卡再单独衡量更小的当前新鲜证据子集。身份总数或历史学费参考都不能替代当前周期完整度，更不能被解释为“都在开放申请”。

## 本轮信任平台里程碑

本轮把“更多记录”推进成“更少决策歧义”：

- 基于官方来源深化浙江大学、湖南工商大学、杭州电子科技大学和广州大学的 **18 个项目**，并补充 6 条奖学金记录；所有已采集的 2026 截止日期都按真实状态标为已截止，不会自动猜成 2027。
- 第二轮官方深度收口覆盖**广东外语外贸大学、上海外国语大学、安徽大学、重庆大学、哈尔滨工业大学和西安交通大学的 34 个项目身份与 11 项奖学金身份**，由幂等 importer 固化适用范围和新鲜度语义。
- 广外 5 份官方资产的校验和与私有 R2 对象键已登记；西交 PDF 的原始字节和浏览器快照未取得，因此所有依赖这些 PDF 的动态事实继续隔离。
- legacy programIds 现在必须通过严格关系校验；奖学金与项目之间缺失或错误的关系不会再被静默接受。
- 项目卡片以中文、英文、俄文、德文、法文和西文展示六种明确事实状态，并同时使用可见文字与形状标记，不只依赖颜色。
- `stale`、冲突、来源不可用与历史周期的动态值会被遮蔽；无日期的参考学费不进入“学费已知”筛选，也不参与价格排序。
- 注册费不会被误写成申请费；学校只公布学费区间时，系统保留官方证据，不会捏造一个项目级单值。

这使平台可以保留稳定的项目发现入口，同时拒绝把旧学费、旧截止日或无法核实的要求包装成当前事实。
...
- 从申请状态与截止日期继续查看学费、授课语言、学制和详情页的申请信息概览；历史学费单独标注学年，仅作预算参考。
...
平台把**记录身份**与**字段可见性**分开管理。一个已确认面向国际生的项目，即使其学费或截止日期当前不能安全展示，也可以作为官方发现入口被保留。公开事实契约支持六种明确状态；项目卡已将它们接入学制、学费、截止日期和申请费：
...
这些状态现在会直接显示在六语项目卡中。旧周期学费可以在当前已确认学费缺失时以“历史参考 + 学年”单独展示，但不能进入当前学费筛选、排序、申请状态、截止日期、申请费或 JSON-LD；申请者仍须进入官方入口最终核验。
...
由采集链路保存的原始 HTML、PDF 与截图进入私有 R2。公开平台只展示通过验证的结构化事实及最短必要来源上下文；“已登记来源”不代表每项资产都已经生成 R2 快照。
...
- 将湖南工商大学从 2 个提升到至少 3 个官方国际生项目；西藏大学在发现另一个明确接受个人申请的官方项目之前，继续保持带证据的 `limited` 状态；浙江大学本轮已形成 6 个可用于决策的项目身份。
- 将新鲜处置覆盖率从 3.33% 提升到至少 70%，不把无日期学费参考计为招生周期。
- 从当前实测的 63.42% 学制、46.56% 官方申请入口、78.31% 授课语言和 11.24% 要求信息，先达到六周门槛 75% / 65% / 90% / 25%，再向 90% / 80% / 95% / 50% 的扩展门槛推进。
- 将关联新鲜核验奖学金的高校从 205 所提升到至少 230 所，并补充真实年度、资助类型、适用范围和截止日期证据。
